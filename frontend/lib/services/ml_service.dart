...
class MLService {
  ...
  Future<void> loadModel() async {
    try {
      ...
      print('Using mock results with ${_labels?.length ?? 0} labels');
    } catch (e) {
  ...
  Future<String> _loadLabelsFromAssets() async {
    try {
      ...
      return await rootBundle.loadString('assets/ml/$LABELS_FILE_NAME');
    } catch (e) {
      print('Labels not found in assets, trying to load from app directory');
      
      // If not in assets, check if it's in the app directory
      final appDir = await getApplicationDocumentsDirectory();
      final labelsFile = File('${appDir.path}/$LABELS_FILE_NAME');
...
''';
  ...
  List<DetectionResult> _generateMockResults() {
    ...
    final healthyIndex = _labels!.indexWhere(
      (label) => label.toLowerCase() == 'healthy'
    );
    ...
    while (results.length < numDetections && usedIndices.length < _labels!.length) {
      int index;
      do {
